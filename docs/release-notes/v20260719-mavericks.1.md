## Docker for Mavericks 20260719-mavericks.1

A packaged, self-updating **Docker toolchain for OS X 10.9 (Mavericks)** — a signed `.pkg`, meant as a
drop-in successor to the hand-built "Docker for Mavericks" DMG, with newer components and an updater.
...
`/usr/local/share/mavericks-docker/README.txt` for the one-line `docker-machine create` setup.
